...
| `cpt-cf-llm-gateway-adr-image-generation-api` | Responses API with custom Gears extensions for image generation |
...
- DataOutput — Gears extension: binary data output (type: "cf-gears:data", id, status: in_progress | completed, mime_type, base64: string | null, url: string | null). Generic binary output item used for image generation and extensible to audio/video. See [ADR-0006](./ADR/0006-cpt-cf-llm-gateway-adr-image-generation-api.md).

Provider-specific items use extension format: `{provider_slug}:{item_type}` (e.g., `openai:web_search_call`). CF/Gears extensions use the `cf_gears:` prefix (e.g., `cf_gears:data`).
...
Tool definitions share a single base type (`Tool`) with GTS type inheritance, discriminated by `type` field. The Open Responses `function` tool type is the standard. Gateway extends with Gears-specific tool types for GTS Type Registry integration.
...
- ToolReference — Gears extension: reference to Type Registry (type: "reference", schema_id). Gateway resolves schema_id via Type Registry before forwarding to provider.
- ToolInlineGTS — Gears extension: inline GTS schema (type: "inline_gts", schema). Gateway resolves GTS schema to JSON Schema before forwarding.
- ImageGenerationTool — Gears extension: built-in image generation tool (type: "cf_gears:image_generation"). Follows the Open Responses hosted tool extension format. Parameters: aspect_ratio (1:1, 2:3, 3:2, 3:4, 4:3, 9:16, 16:9), resolution (megapixels: 0.5, 1, 2, 4), quality (low, medium, high, auto), output_format (png, jpeg, webp), output_compression (0–100, for jpeg), response_format (base64, url). See [ADR-0006](./ADR/0006-cpt-cf-llm-gateway-adr-image-generation-api.md).
...
**Components**:
 ...
 - [ ] `p1` - **ID**: `cpt-cf-llm-gateway-component-hook-plugin`
   - Hook Plugin
   - Pre-call and post-response interception following Gears plugin architecture. Multiple plugins can be enabled and are invoked in order. Pre-call plugins can modify or block requests before the provider adapter. Post-call plugins run after the full response is available (after streaming completes, or after async/batch job finishes) and are observe-only — the response has already been delivered or is delivered unconditionally, so plugin outcome has no effect on response delivery.
...
| `cf_gears:response.data.in_progress` | Gears extension: binary data output item started processing (e.g., image generation in progress) |
| `cf_gears:response.data.done` | Gears extension: binary data output item completed with final data (base64 or URL) |
...
- Gears extension events use `cf_gears:` prefix: `cf_gears:response.data.in_progress` (binary output started), `cf_gears:response.data.done` (binary output completed with data)
...
**Technology**: ToolKit SDK trait (`LlmGatewayHookPluginClient`), resolved via ClientHub scoped clients following Gears plugin architecture (see `docs/TOOLKIT_PLUGINS.md`)

**GTS Schema ID**: `gts.cf.toolkit.plugins.plugin.v1~cf.llmgw.hook_plugin.v1~`
...
`ProviderCallCtx` is a core-owned, per-call context passed to every method: the resolved `provider_model_id`, the provider identity (`provider_type`, the Model Registry model-info `gts_type`), an opaque `provider_settings` payload the plugin narrows to its own typed settings (carrying its OAGW routing handle, e.g. an `oagw_alias`), and the request correlation id. This lets the plugin operate without its own Model Registry access.
