...
- ~~**Lifecycle-filter escape hatch** — fixed.`list_tenant_models` now ANDs the lifecycle exclusion unconditionally on the eval path (§3.3). The string-matching branch that suppressed the exclusion when `lifecycle_status` was mentioned in the filter is deleted; the management path uses the `include_deprecated` flag instead.~~
- ~~**Model merge keyed on `canonical_id`** — fixed.`list_tenant_models` now carries `provider_id IN allow_list` as its visibility predicate, keyed on the §3.5 `ChainProviders` primitive, with `canonical_id` dedupe kept only as a documented redundant safety net. `get_tenant_model` resolves the slug closest-first and probes under the winning tenant only, with gate ordering per §3.5 (provider mismatch → lifecycle → provider status).~~
