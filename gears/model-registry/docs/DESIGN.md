...
- [x] `p1` — `cpt-cf-model-registry-fr-cache-isolation` — Cache key format `mr:{tenant_id}:{entity}:{id}`, one `cache_ttl_seconds` expiry on every entry, `invalidate_tenant` scoped by key prefix
...
|--------|-------------|--------------|-----------------|-----------|----------------------|
| `cpt-cf-model-registry-nfr-performance` | get_tenant_model <10ms P99 | Cache Layer + Repository | Cache-first single-entity reads with a single TTL on every entry (`cache_ttl_seconds`, default 10 min). List reads are **not** cached (§2.1) and meet their target through indexed columns and the per-ancestor query path | Implemented against the in-process `InMemoryCache` — the only P1 backend; adopting a distributed backend is an open question (§4 Technical Debt & Roadmap) | Benchmarks deferred — no measured P99 yet (§4 Technical Debt) |
