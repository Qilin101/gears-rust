...
Read operations check the cache before the database. *Which* tenants get probed depends on the entity, and the difference is load-bearing: provider-slug ownership is resolved by walking the chain closest-first and stopping at the first owner, whereas a model is read under the winning provider's tenant **only** — probing the chain closest-first for a model and returning on the first hit would serve a cached ancestor row to a subtree that has shadowed its provider, since the hit path performs no slug resolution (§3.5 sub-decision 2). Cache misses populate the cache from DB. TTL-based expiry prevents stale data accumulation. **One TTL governs every entry** (`cache_ttl_seconds`, default 10 minutes). An earlier design split it by ownership — longer for own data, shorter for inherited — but a cache key is prefixed by the tenant that *owns* the row, so a single entry is shared by that tenant and its entire subtree and cannot carry two different expiries; honouring both would require a reader-side staleness bound on every read. The split also had no mechanism behind it: within a process every write drops the owning tenant's whole prefix for owner and descendants alike, so TTL only backstops cross-replica and out-of-band changes, which are symmetric between the two ownership classes. `Ownership` still classifies rows for visibility merging; it no longer selects a TTL. The backend sits behind the `CacheService` trait — P1 ships `InMemoryCache` only, and a distributed backend plugs in behind the declared `redis` Cargo feature without touching the service.
...
**Description**: Discovers models via the registered discovery plugin matching the provider's GTS type, then reconciles the plugin's model definitions against the catalog (new models as `pending`, existing models updated, absent models deprecated), and invalidates cache for the owner tenant. Child tenants that inherit these models are **not** explicitly invalidated — they rely on the TTL to pick up changes. (Since keys are prefixed by the owning tenant, invalidating the owner does in fact drop the entries descendants read; the TTL matters only where that invalidation cannot reach, i.e. another replica.) Explicitly invalidating all descendant caches would require traversing the tenant tree on every discovery run.
...
- **Cross-replica cache TTL trade-off**: within a process, owner-prefixed keys mean a write reaches descendants without a subtree walk. Across replicas nothing propagates, so any reader trails by up to `cache_ttl_seconds`; closing that needs the distributed backend behind the `redis` feature, not an O(tenant-tree) walk.
