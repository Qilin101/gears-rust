...
- `$select` is **not supported**. The listing endpoints reject a request carrying it with a `400` field violation on `$select` (reason `UNSUPPORTED_SELECT`): responses are whole `ModelDto` / `ProviderDto` values and there is no projection stage, so silently ignoring the clause would return more than the caller asked for. The SDK surface cannot honour it either — `list_tenant_models` / `list_providers` return typed `Page<ModelV1>` / `Page<ProviderV1>`.
